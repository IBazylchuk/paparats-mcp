...
function paparatsService(port: number, ollamaUrl: string): ComposeService {
  return {
...
export function generateDockerCompose(config: DockerComposeConfig): string {
  ...
  const compose: ComposeFile = {
    services: {
      ...
      paparats: paparatsService(paparatsPort, ollamaUrl),
    },
    volumes: {
      qdrant_data: null,
      paparats_data: null,
...
export function generateServerCompose(config: ServerComposeConfig): string {
  ...
  // paparats depends on both qdrant and ollama in server mode
  compose.services['paparats']!.depends_on!['ollama'] = { condition: 'service_healthy' };
